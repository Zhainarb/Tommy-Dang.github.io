/**
 * Copyright (c) 2010 Leland Wilkinson
 */
package main;
...
public class ColorScales {
    ...
    public static final String GRAY = "gray", RAINBOW = "rainbow", ALTITUDE = "altitude",
            ...
            RED = "red", BLUE = "blue", GREEN = "green", CATEGORICAL = "categorical", CYAN ="cyan";
    public static final String[] scalesList = { RAINBOW,GRAY, ALTITUDE, 
    	CIRCULAR, BIPOLAR, REDBLUE, REDGREEN, BLUEGREEN, HEAT, LIGHT, RED, BLUE, GREEN};
    ...
    private static Color rainbow(double value, float opacity) {
        ...
        if (v < vmin + 0.25f * range)
            return new Color(0.f, 4.f * (v - vmin) / range, 1.f, opacity);
        else if (v < vmin + 0.5 * range)
            return new Color(0.f, 1.f, 1.f + 4.f * (vmin + 0.25f * range - v) / range, opacity);
        else if (v < vmin + 0.75 * range)
            return new Color(4.f * (v - vmin - 0.5f * range) / range, 1.f, 0, opacity);
        else
            return new Color(1.f, 1.f + 4.f * (vmin + 0.75f * range - v) / range, 0, opacity);
    }
    ...
    private static Color cyan(double value, float transparency) {
    	float v = (float) value;
    	return new Color(0.4f+v*0.6f,0.2f, 0.4f+v*0.6f, transparency);
    }
