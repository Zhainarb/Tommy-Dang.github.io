...
class Triangulation {
    boolean ALPHA = true;      // set Edge.onShape=false before using
    boolean GABRIEL = false;   // set Edge.onShape=true before using
    boolean RNG = false;       // set Edge.onShape=true before using
    boolean NNG = false;       // set Edge.onShape=false before using
    boolean DIST = false;      // set Edge.onShape=false before using
    boolean SPHERE = false;    // set Edge.onShape=false before using
    ...
    public double[] compute(boolean[][] bdata, int file) {
    	// Compute total count
    	fileId = file;
    	totalCount =0;
       ...
       if (totalCount< main.MainSETI_3_6.minPoints ){
        	double[] measures = {level,level,0,0,0,level,0,level};
        	return measures;
        }
        else if (level > 0.5){
        	double[] measures = {level,level,0,0,0,level,0,level};
    	...
    	px = new int[totalCount];
    	py = new int[totalCount];
    	int ccc = 0;
    	for (int i = 0; i < size; i++) {
        	for (int j = 0; j < size; j++) {
                if (bdata[i][j]){
    ...
    private double[] computeMeasures() {
        double[] results = new double[main.MainSETI_3_6.nS];
        // Do not change order of these calls!
        results[main.MainSETI_3_6.PIXELLEVEL] = level;
        //results[main.MainSETI_Version_2_2.OUTLYING] = computeOutlierMeasure();
        results[main.MainSETI_3_6.CLUMPY] = computeClusterMeasure();
        //results[main.MainSETI_Version_2_2.SKEWED] = computeMSTEdgeLengthSkewnessMeasure();
        //results[main.MainSETI_2_4.CONVEX] = computeConvexityMeasure();
        results[main.MainSETI_3_6.SKINNY] = computeSkinnyMeasure();
        results[main.MainSETI_3_6.STRINGY] = computeStringyMeasure();
        results[main.MainSETI_3_6.STRIATED] = computeStriationMeasure();
        results[main.MainSETI_3_6.SPARSE] = computeSparsenessMeasure();
        results[main.MainSETI_3_6.DENSE] = computeDenseMeasure();
        results[main.MainSETI_3_6.MONOTONIC] = computeMonotonicityMeasure();
        return results;
    }
    ...
    private void computeDT(int[] px, int[] py) {
        ...
        for (int i = 0; i < px.length; i++) {
            int x = px[i] + (int) (5 * (r.nextDouble() - .5)); // perturb to prevent singularities
            int y = py[i] + (int) (5 * (r.nextDouble() - .5));
            int count = 1;
            if (!isOutlier[i]) {
                insert(x, y, count, i);
    ...
    private double computeCutoff(double[] lengths) {
        ...
        int n25 = n50 / 2;
        int n75 = n50 + n25;
        return lengths[n75] + 0.10 * (lengths[n75] - lengths[n25]);
    }
    ...
    private boolean pointsInLune(Edge e, double radius) {
        double r = FUZZ * radius;
    ...
    private double computeStringyMeasure() {
        ...
        if (totalCount<25){
        	result = result*(((double)totalCount-5)/20);
        }
    ...
    private double computeClusterMeasure() {
        ...
        double clumpy = 2 * maxValue / totalPeeledCount;
        //Tuan
    	if (totalCount<25){
        	clumpy = clumpy*(((double)totalCount-5)/20);
        }
        return clumpy;
    }
    ...
    private double computeMonotonicityMeasure() {
        ...
        double monotonic = s*s;
      //Tuan
    	if (totalCount<25){
    		monotonic = monotonic*(((double)totalCount-5)/20);
        }
        return monotonic;
    }
    ...
    private double computeSparsenessMeasure() {
        ...
        // Tuan
        double val = sortedOriginalMSTLengths[n45];
        double sparse = (150-val)/150;
        if (sparse<0) 
        	sparse = 0;
        return sparse;
    }
    
    private double computeDenseMeasure() {
    	double dense = alphaArea;
    	//Tuan
    	if (totalCount<25){
    		dense = dense*(((double)totalCount-5)/20);
        }
    ...
    private double computeStriationMeasure() {
        ...
        double striated = numEdges*1.3f / (double) mstEdges.size();
        
        //Tuan
    	if (totalCount<25){
    		striated = striated*(((double)totalCount-5)/20);
        }
    ...
    private double computeSkinnyMeasure() {
        if (alphaPerimeter > 0){
            double skinny = 1 - Math.sqrt(4 * Math.PI * alphaArea) / alphaPerimeter;
        	//Tuan
        	if (totalCount<25){
        		skinny = skinny*(((double)totalCount-5)/20);
            }
